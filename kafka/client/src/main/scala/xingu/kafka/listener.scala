...
import xingu.kafka.api.KafkaMessagePointer
...
@Singleton
class SimpleKafkaListener @Inject()(services : Services, ws: WSClient) extends KafkaListener {
  ...
  if(enabled) {
    supervisor = services.actorSystem().actorOf(Props(classOf[KafkaSupervisor], services, ws), "kafka-supervisor")
    supervisor ! Refresh
  } else {
...
class ConsumerFactory (conf: Configuration) {
  ...
  val logger       = LoggerFactory.getLogger(getClass)
  val timeout      = conf.getOptional[Duration]("timeout").getOrElse(5 minutes)
  val alwaysCommit = conf.get[Boolean]("alwaysCommit")
  ...
  def createConsumer(from: Option[LocalDateTime] = None) = Try {
    val group   = conf.get[String]      ("group")
    val topics  = conf.get[Seq[String]] ("topics")
    ...
    val props = Map(
      "client.id"                             -> "oystr-kafka-consumer",
      "group.id"                              -> group,
...
class KafkaSupervisor(services: Services, ws: WSClient) extends Actor with Timers {
  ...
  val conf         = services.conf().get[Configuration]("xingu.kafka.consumer")
  val factory      = new ConsumerFactory(conf)
  val timeout      = java.time.Duration.ofMillis(factory.timeoutInMillis)
  val refreshEvery = conf.getOptional[FiniteDuration]("refreshEvery").getOrElse(1 hour)
  val handler      = services.actorSystem().actorOf(Props(classOf[EventHandler], services, ws), "kafka-event-handler")
...
class EventHandler(services: Services, handler: MessageHandler, ws: WSClient) extends Actor {

  private val logger = LoggerFactory.getLogger(getClass)

  private val storage = StorageClient(services.conf(), ws)
...
@Singleton
class RouteMessageHandler @Inject() (services: Services, ws: WSClient) extends MessageHandler {

  private val logger = LoggerFactory.getLogger(getClass)

  private val routes = routesGiven(services.conf().get[Seq[Configuration]]("kafka-consumer.routes"))
